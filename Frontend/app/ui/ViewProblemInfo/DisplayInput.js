...
const DisplayInput = ({ name, value }) => {
  const valueType = useMemo(() => typeOfExtended(value), [value]);
  const textClass =
    "text-xs md:text-sm xl:text-base text-textBackground dark:text-dark-textBackground";
        ...
        <>
          ...
          <div className="ml-2 flex flex-col gap-1 md:gap-2">
            {displayedValue.map((v, index) => (
              <DisplayInput key={`${name}-${index}`} name={null} value={v} />
            ))}
